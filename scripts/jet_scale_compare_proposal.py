...
def prepare(name,input_file, other1, other2, rho0, rho1, folder, pt=True):
    ...
    for i in range(0,len(parameter)-1):
        ...
        mean, mean_error, sigma, sigma_error = ConvFit(shape ,False,"ratio","jet pt/gen jet pt",folder+"/fit","FIT_"+name+"_pf_"+str(parameter[i])+"_"+str(other1))
...
def main():
    ...
    h_mean , h_sigma , h_sigma11 = prepare("sron","../output/QCD_FlatPt_v2_SRon_numEvent10000.root",_other[0],_other[1],_rho[0],_rho[1], folder,pt=True)
    h_mean2, h_sigma2, h_sigma21 = prepare("sroff","../output/QCD_FlatPt_v2_SRoff_numEvent10000.root",_other[0],_other[1],_rho[0],_rho[1], folder,pt=True)
    h_mean3, h_sigma3, h_sigma31 = prepare("proposal","../output/QCD_FlatPt_v2_SRProposal.root",_other[0],_other[1],_rho[0],_rho[1], folder,pt=True)
    ...
    h_mean2.SetLineWidth(2)
    h_mean2.Draw("lsame")
    h_mean3.SetLineColor(2)
    h_mean3.SetLineWidth(2)
    h_mean3.Draw("lsame")

    legend = ROOT.TLegend(0.30, 0.65, 0.65, .85);
    legend . AddEntry(h_mean, "SR on" , "l")
    legend . AddEntry(h_mean2,"SR off" , "l")
    legend . AddEntry(h_mean3,"Proposal" , "l")
    ...
    h_sigma2.SetLineWidth(2)
    h_sigma2.Draw("lsame")
    h_sigma3.SetLineColor(2)
    h_sigma3.SetLineWidth(2)
    h_sigma3.Draw("lsame")
    ...
    h_sigma21.SetLineWidth(2)
    h_sigma21.Draw("lsame")
    h_sigma31.SetLineColor(2)
    h_sigma31.SetLineWidth(2)
